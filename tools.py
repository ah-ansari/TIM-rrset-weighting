...
def one_round_diffuse(g: nx.Graph, seed_set1, seed_set2):
    counter = 0
...
def action_degree(g: nx.Graph, k):
    free_nodes = g.graph['free']
    degrees = g.out_degree(free_nodes)
    degrees_sorted = sorted(degrees.items(), key=operator.itemgetter(1), reverse=True)
    seed_set = []
    for i in range(k):
        seed_set.append(degrees_sorted[i][0])
